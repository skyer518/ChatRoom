...
function Server (options, origins) {
  ...
  this.socket.on('error', function serverError (err) {
    ...
    if (err.errno == 13) {
      me.log && me.log(
        'Unable to listen to port `' + me.port + '` as your Node.main instance does not have root privileges. ' +
        (
          me.server
...
/**
 * Compiles the supplied origins to a Flash Policy File format and stores it in a Node.main Buffer
